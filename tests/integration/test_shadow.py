...
def test_unreadable_files_do_not_fail_the_snapshot(project: Path) -> None:
    ...
    try:
        ...
        assert "unreadable.txt" not in _shadow_ls(store.dir, snap.tree)
        assert snap.unreadable == ("unreadable.txt",)
    finally:
        secret.chmod(0o600)
...
def test_git_failure_other_than_corruption_keeps_the_index(
    ...
    original = ShadowGit.run_process

    def slow_add(
        self: ShadowGit, *args: str, **kwargs: object
    ) -> subprocess.CompletedProcess[bytes]:
        if "add" in args:
            raise GitError(args, -1, "timed out after 30.0s")
        return original(self, *args, **kwargs)  # type: ignore[arg-type]

    monkeypatch.setattr(ShadowGit, "run_process", slow_add)
    with pytest.raises(GitError, match="timed out"):
        write_tree(store, "s1", Limits())
    assert index.exists()


def test_unreadable_path_parsing_accepts_only_read_errors() -> None:
    from agent_blackbox.core.errors import GitError  # noqa: PLC0415
    from agent_blackbox.core.snapshot import _unreadable_paths  # noqa: PLC0415

    ok = b"error: open(\"a b.txt\"): Permission denied\nerror: unable to index file 'a b.txt'\n"
    assert _unreadable_paths(ok) == ("a b.txt",)
    with pytest.raises(GitError):
        _unreadable_paths(b"fatal: Unable to create '/x/index.lock': File exists.\n")
    with pytest.raises(GitError):
        _unreadable_paths(ok + b"fatal: something else\n")
    with pytest.raises(GitError):
        _unreadable_paths(b"")
